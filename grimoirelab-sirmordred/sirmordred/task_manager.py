...
class TasksManager(threading.Thread):
    ...
    def run(self):
        ...
        # 当update设为fasle时，最开始，stopper 为false，self.stopper.is_set() 返回false ,while循环条件就为true，执行
        # 执行完一遍之后，sirmordred.py 那里会调用 set(),将stopper设为 false，循环条件就不满足，退出，达到了只执行一次的效果
        # 当update设为true时，会一直循环下去
