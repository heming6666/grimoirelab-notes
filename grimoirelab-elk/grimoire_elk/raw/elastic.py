...
class ElasticOcean(ElasticItems):
    ...
    def feed(self, from_date=None, from_offset=None, category=None, branches=None,
        ...
        # 主要调用,从github拿数据
        # perceval_backend 即为 Github 类的一个实例
        # 注意是先调用的 Github 类的 fetch(),而不是GithubClient类的 
    ...
    def _items_to_es(self, json_items):
        ...
        if len(json_items) == 0:
            return
        # log: [github] Adding items to Raw for <grimoire_elk.raw.github.GitHubOcean object at 0x7f4b3e209588> (1 items)
        logger.debug("[{}] Adding items to Raw for {} ({} items)".format(
                     self.perceval_backend.__class__.__name__.lower(),
                     self, len(json_items)))

        field_id = self.get_field_unique_id()
        # 上传,调用elastic client的bulk_upload,在../elastic.py
